...
namespace Transight.Interface.Config
{
    ...
    public static class AppConfig
    {
        ...
        private static string startDate; // reading start date
        private static int pc_seq; //pc seq
        private static string sentLogs; //view logs
        private static string autoTimer;
        private static string schedType;
        private static string schedInterval;
        //other configs
        ...
        public static string Sent_Logs
        {
            get
            {
                return sentLogs;
        ...
        public static string _autoTimer
        {
            get
            {
                return autoTimer;
        ...
        public static string _schedType
        {
            get
            {
                return schedType;
        ...
        public static string _schedInterval
        {
            get
            {
        ...
        public static void Load(bool ShowMessage)
        {
            try
            {
                ...
                sConfig = ConfigurationManager.AppSettings["DBUserName"];
                if (sConfig == null) throw new ApplicationException("Error getting Transight DB User Name");
                dbUserName = sConfig.Trim();

                sConfig = ConfigurationManager.AppSettings["DBPassword"];
                if (sConfig == null) throw new ApplicationException("Error getting Transight DB Password");
                dbPassword = sConfig.Trim();
                ...
                sConfig = ConfigurationManager.AppSettings["ZeroStartDate"];
                if (sConfig == null) throw new ApplicationException("Error getting Start Date");
                startDate = sConfig.Trim();

                sConfig = ConfigurationManager.AppSettings["PC_SEQ"];
                if (sConfig == null || !Func.IsInt32(sConfig.Trim())) throw new ApplicationException("Error getting pc_seq");
                pc_seq = Int32.Parse(sConfig.Trim());

                sConfig = ConfigurationManager.AppSettings["AutoTimer"];
                if (sConfig == null) throw new ApplicationException("Error getting POS Config Option");
                autoTimer = sConfig.Trim();

                sConfig = ConfigurationManager.AppSettings["SchedType"];
                if (sConfig == null) throw new ApplicationException("Error getting POS Config Option");
                schedType = sConfig.Trim();

                sConfig = ConfigurationManager.AppSettings["SchedInterval"];
                if (sConfig == null) throw new ApplicationException("Error getting POS Config Option");
                schedInterval = sConfig.Trim();
        ...
        public static bool SaveAll(bool IsRegistry, string NewDBServer, string NewDBName, string NewDBUserName,
                string NewDBPassword, string NewLogPath, int NewLogCleanUp, int NewBusinessDateOffset, string NewZeroStartDate, int Pc_Seq,
                string NewAutoTimer, string NewScheduleType, string NewSchedInterval, List<XMLConfig> NewXMLConfigList)
        {
            try
            {
                ...
                config.AppSettings.Settings["DBUserName"].Value = NewDBUserName;
                config.AppSettings.Settings["DBPassword"].Value = NewDBPassword;
                ...
                config.AppSettings.Settings["ZeroStartDate"].Value = NewZeroStartDate;
                config.AppSettings.Settings["PC_SEQ"].Value = Pc_Seq.ToString();
                config.AppSettings.Settings["AutoTimer"].Value = NewAutoTimer;
                config.AppSettings.Settings["SchedType"].Value = NewScheduleType;
                config.AppSettings.Settings["SchedInterval"].Value = NewSchedInterval;
                //set new value
                if (IsRegistry)
                    dbOption = TransightDBOption.Registry;
                else
                ...
                startDate = NewZeroStartDate;
                pc_seq = Pc_Seq;
                autoTimer = NewAutoTimer;
                schedInterval = NewSchedInterval;
                schedType = NewScheduleType;
                //set other config
